...
const UploadSelfie = () => {
    ...
    useEffect(() => {
        // Check AWS credentials and initialize component
        const initializeComponent = async () => {
            try {
                ...
                // Only redirect to events page if there's no event ID in the URL
                if (!window.location.pathname.includes('upload-selfie')) {
                    navigate('/events');
                }
            }
        };
    ...
    const compareFaces = async (selfieUrl: string) => {
        try {
            ...
            const listCommand = new ListObjectsV2Command({
                ...
                Prefix: `events/${userEmail}/${currentEventId}/images/`,
            });
